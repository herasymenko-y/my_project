...
static int affs_remove_link(struct dentry* dentry) {
  ...
  if (inode->i_ino == link_ino) {
    /* we can't remove the head of the link, as its blocknr is still used as
     * ino, so we remove the block of the first link instead.
     */
...
void affs_mode_to_prot(struct inode* inode) {
   ...
   */
  prot &=
      ~(FIBF_NOEXECUTE | FIBF_NOREAD | FIBF_NOWRITE | FIBF_NODELETE |
        FIBF_GRP_EXECUTE | FIBF_GRP_READ | FIBF_GRP_WRITE | FIBF_GRP_DELETE |
