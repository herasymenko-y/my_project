...
struct inode* affs_iget(struct super_block* sb, unsigned long ino) {
  ...
  switch (be32_to_cpu(tail->stype)) {
    ...
    case ST_FILE:
      ...
      if (inode->i_size) {
        AFFS_I(inode)->i_blkcnt = (size - 1) / sbi->s_data_blksize + 1;
        AFFS_I(inode)->i_extcnt =
            (AFFS_I(inode)->i_blkcnt - 1) / sbi->s_hashsize + 1;
      }
      if (tail->link_chain)
        set_nlink(inode, 2);
      inode->i_mapping->a_ops =
          affs_test_opt(sbi->s_flags, SF_OFS) ? &affs_aops_ofs : &affs_aops;
...
int affs_write_inode(struct inode* inode, struct writeback_control* wbc) {
  ...
  if (tail->stype == cpu_to_be32(ST_ROOT)) {
    ...
    affs_secs_to_datestamp(inode_get_mtime_sec(inode), &tail->change);
...
int affs_setattr(struct mnt_idmap* idmap,
