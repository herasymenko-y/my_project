...
static struct buffer_head* affs_alloc_extblock(struct inode* inode,
                                               struct buffer_head* bh,
                                               u32 ext) {
  struct super_block* sb = inode->i_sb;
...
static inline struct buffer_head* affs_get_extblock(struct inode* inode,
                                                    u32 ext) {
...
static struct buffer_head* affs_get_extblock_slow(struct inode* inode,
                                                  u32 ext) {
  struct super_block* sb = inode->i_sb;
  struct buffer_head* bh;
  u32 ext_key;
...
static int affs_get_block(struct inode* inode,
                          sector_t block,
  ...
  if (create) {
    ...
    if (bh_result->b_blocknr)
      affs_warning(sb, "get_block", "block already set (%llx)",
...
static ssize_t affs_direct_IO(struct kiocb* iocb, struct iov_iter* iter) {
...
static int affs_write_begin(const struct kiocb* iocb,
  ...
  ret = cont_write_begin(iocb, mapping, pos, len, foliop, fsdata,
                         affs_get_block, &AFFS_I(mapping->host)->mmu_private);
  if (unlikely(ret))
...
static inline struct buffer_head* affs_bread_ino(struct inode* inode,
                                                 int block,
                                                 int create) {
...
static inline struct buffer_head* affs_getzeroblk_ino(struct inode* inode,
                                                      int block) {
  struct buffer_head *bh, tmp_bh;
  int err;
...
static inline struct buffer_head* affs_getemptyblk_ino(struct inode* inode,
                                                       int block) {
  struct buffer_head *bh, tmp_bh;
  int err;
...
static int affs_do_read_folio_ofs(struct folio* folio, size_t to, int create) {
  ...
  pr_debug("%s(%llu, %ld, 0, %zu)\n", __func__, inode->i_ino, folio->index, to);
...
static int affs_extent_file_ofs(struct inode* inode, u32 newsize) {
  struct super_block* sb = inode->i_sb;
    ...
    if (prev_bh) {
      ...
      if (tmp_next)
        affs_warning(sb, "extent_file_ofs",
                     "next block already set for %d (%d)", bidx, tmp_next);
...
static int affs_write_begin_ofs(const struct kiocb* iocb,
  ...
  pr_debug("%s(%llu, %llu, %llu)\n", __func__, inode->i_ino, pos, pos + len);
...
static int affs_write_end_ofs(const struct kiocb* iocb,
  ...
  if (boff) {
    ...
    AFFS_DATA_HEAD(bh)->size =
        cpu_to_be32(max(boff + tmp, be32_to_cpu(AFFS_DATA_HEAD(bh)->size)));
    affs_fix_checksum(sb, bh);
    mark_buffer_dirty(bh);
    written += tmp;
    from += tmp;
...
void affs_truncate(struct inode* inode) {
  ...
  if (inode->i_size > AFFS_I(inode)->mmu_private) {
    ...
    if (!res)
      res =
          mapping->a_ops->write_end(NULL, mapping, isize, 0, 0, folio, fsdata);
    else
    ...
    if (affs_test_opt(AFFS_SB(sb)->s_flags, SF_OFS)) {
      ...
      if (IS_ERR(bh)) {
        affs_warning(sb, "truncate",
                     "unexpected read error for last block %u (%ld)", ext,
                     PTR_ERR(bh));
        return;
      }
