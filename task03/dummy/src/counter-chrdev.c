...
#define counter_comp_read_is_equal(a, b)                                     \
  (a.action_read == b.action_read || a.device_u8_read == b.device_u8_read || \
...
#define counter_comp_read_is_set(comp)                                    \
  (comp.action_read || comp.device_u8_read || comp.count_u8_read ||       \
   ...
   comp.signal_u32_read || comp.device_u64_read || comp.count_u64_read || \
   comp.signal_u64_read || comp.signal_array_u32_read ||                  \
   comp.device_array_u64_read || comp.count_array_u64_read ||             \
   comp.signal_array_u64_read)

static ssize_t counter_chrdev_read(struct file* filp,
                                   char __user* buf,
                                   size_t len,
                                   loff_t* f_ps) {
  ...
  do {
    if (kfifo_is_empty(&counter->events)) {
      ...
      err = wait_event_interruptible(
          counter->events_wait,
          !kfifo_is_empty(&counter->events) || !counter->ops);
      if (err < 0)
        return err;
      if (!counter->ops)
        return -ENODEV;
...
static int counter_set_event_node(struct counter_device* const counter,
  ...
  list_for_each_entry(event_node, &counter->next_events_list,
                      l) if (event_node->event == watch->event &&
                             event_node->channel == watch->channel) break;
  ...
  list_for_each_entry(
      comp_node, &event_node->comp_list,
      l) if (comp_node->parent == cfg->parent &&
...
static long counter_chrdev_ioctl(struct file* filp,
                                 unsigned int cmd,
                                 unsigned long arg) {
  struct counter_device* const counter = filp->private_data;
  int ret = -ENODEV;
  ...
  switch (element->type) {
    case COUNTER_COMP_SIGNAL_POLARITY:
      if (scope != COUNTER_SCOPE_SIGNAL)
        return -EINVAL;
      ret = comp->signal_array_u32_read(counter, parent, idx, &value_u32);
      *value = value_u32;
      return ret;
    case COUNTER_COMP_U64:
      switch (scope) {
        ...
        case COUNTER_SCOPE_SIGNAL:
          return comp->signal_array_u64_read(counter, parent, idx, value);
        case COUNTER_SCOPE_COUNT:
          return comp->count_array_u64_read(counter, parent, idx, value);
        default:
          return -EINVAL;
      }
    default:
...
static int counter_get_data(struct counter_device* const counter,
  ...
  switch (comp->type) {
    ...
    case COUNTER_COMP_SIGNAL_POLARITY:
      switch (scope) {
        case COUNTER_SCOPE_DEVICE:
          ...
          break;
        case COUNTER_SCOPE_SIGNAL:
          ret = comp->signal_u32_read(counter, signal, &value_u32);
          break;
        case COUNTER_SCOPE_COUNT:
    ...
    case COUNTER_COMP_ARRAY:
      ...
      return counter_get_array_data(counter, scope, comp_node->parent, comp,
                                    id - ext_id, value);
    default:
      return -EINVAL;
  }
...
void counter_push_event(struct counter_device* const counter,
  ...
  /* Search for event in the list */
  list_for_each_entry(event_node, &counter->events_list,
                      l) if (event_node->event == event &&
                             event_node->channel == channel) break;
  ...
  list_for_each_entry(comp_node, &event_node->comp_list, l) {
    ...
    copied += kfifo_in_spinlocked_noirqsave(&counter->events, &ev, 1,
                                            &counter->events_in_lock);
  }
