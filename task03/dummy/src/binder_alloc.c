...
static void binder_lru_freelist_add(struct binder_alloc* alloc,
  ...
  for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
    ...
    ret = list_lru_add(alloc->freelist, page_to_lru(page), page_to_nid(page),
                       NULL);
    WARN_ON(!ret);
...
static struct page* binder_page_lookup(struct binder_alloc* alloc,
  ...
  if (binder_alloc_is_mapped(alloc))
    npages = get_user_pages_remote(mm, addr, 1, FOLL_NOFAULT, &page, NULL);
  mmap_read_unlock(mm);
  ...
  switch (ret) {
    ...
    default:
      ...
      pr_err("%d: %s failed to insert page at offset %lx with %d\n", alloc->pid,
             __func__, addr - alloc->vm_start, ret);
      break;
  }
out:
...
static void binder_lru_freelist_del(struct binder_alloc* alloc,
  ...
  for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
    ...
    if (page) {
      ...
      on_lru = list_lru_del(alloc->freelist, page_to_lru(page),
...
static void debug_no_space_locked(struct binder_alloc* alloc) {
  ...
  binder_alloc_debug(BINDER_DEBUG_USER_ERROR,
                     "allocated: %zd (num: %zd largest: %zd), free: %zd (num: "
                     "%zd largest: %zd)\n",
...
struct binder_buffer* binder_alloc_new_buf(struct binder_alloc* alloc,
  ...
  if (unlikely(!size)) {
    binder_alloc_debug(BINDER_DEBUG_BUFFER_ALLOC,
                       ...
                       alloc->pid, data_size, offsets_size, extra_buffers_size);
...
err_already_mapped:
  mutex_unlock(&binder_alloc_mmap_lock);
err_invalid_mm:
  binder_alloc_debug(BINDER_DEBUG_USER_ERROR, "%s: %d %lx-%lx %s failed %d\n",
...
void binder_alloc_deferred_release(struct binder_alloc* alloc) {
  ...
  while (!list_empty(&alloc->buffers)) {
    buffer = list_first_entry(&alloc->buffers, struct binder_buffer, entry);
    WARN_ON(!buffer->free);
  ...
  if (alloc->pages) {
    ...
    for (i = 0; i < alloc->buffer_size / PAGE_SIZE; i++) {
      ...
      on_lru = list_lru_del(alloc->freelist, page_to_lru(page),
                            page_to_nid(page), NULL);
      binder_alloc_debug(BINDER_DEBUG_BUFFER_ALLOC, "%s: %d: page %d %s\n",
                         __func__, alloc->pid, i, on_lru ? "on lru" : "active");
      binder_free_page(page);
      page_count++;
    }
  }
  ...
  binder_alloc_debug(BINDER_DEBUG_OPEN_CLOSE, "%s: %d buffers %d, pages %d\n",
...
void binder_alloc_print_allocated(struct seq_file* m,
  ...
  for (n = rb_first(&alloc->allocated_buffers); n; n = rb_next(n)) {
    ...
    seq_printf(m, "  buffer %d: %lx size %zd:%zd:%zd %s\n", buffer->debug_id,
               buffer->user_data - alloc->vm_start, buffer->data_size,
               buffer->offsets_size, buffer->extra_buffers_size,
...
enum lru_status binder_alloc_free_page(struct list_head* item,
                                       ...
                                       void* cb_arg) __must_hold(&lru->lock) {
...
int binder_alloc_copy_from_buffer(struct binder_alloc* alloc,
                                  ...
                                  binder_size_t buffer_offset,
                                  size_t bytes) {
  return binder_alloc_do_buffer_copy(alloc, false, buffer, buffer_offset, dest,
                                     bytes);
}
